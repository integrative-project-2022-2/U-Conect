...
async function api(req, res) {
    if(req.method === 'PUT') {
        ...
        const data = await db.query('SELECT * FROM ACTIVITY WHERE idActivity = $1', [req.body.id]);
        return res.json(data.rows)
    } else if (req.method === 'POSTPARTICIPANTS'){
        const data = await db.query('SELECT * FROM PARTICIPANT WHERE idActivity = $1', [id])
    }
}
